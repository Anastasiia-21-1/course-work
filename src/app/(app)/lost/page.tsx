'use client'
import {useGetLostsPaged} from "@/api/losts";
...
export default function LostPage() {
    ...
    const {data, isLoading, error} = useGetLostsPaged(params);
    ...
    return (
        <AppContainer title="Всі втрати">
            <div className="mb-4 flex flex-wrap items-end gap-2">
                ...
                <select
                    value={cityId ?? ''}
                    onChange={(e) => updateParams({ cityId: e.target.value ? Number(e.target.value) : undefined })}
                    className="border rounded px-3 py-2"
                >
                ...
                <select
                    value={categoryId ?? ''}
                    onChange={(e) => updateParams({ categoryId: e.target.value ? Number(e.target.value) : undefined })}
                    className="border rounded px-3 py-2"
                >
                ...
                >
                    {[6, 12, 24, 48].map((n) => (
                        <option key={n} value={n}>{n} на стор.</option>
                    ))}
                </select>
